...
\begin{footnotesize}
\begin{longtable}{c  c  p{6cm}  p{6cm} c c}
    ...
    \makecell[b]{Sigla\\ da \\Disciplina}
        ...
        &   \makecell[b]{Atividades Atribuidas}
        &   \makecell[b]{Monitores Solicitados}
...
@foreach($schoolterm->schoolclasses()->whereHas('department', function ($query) { return $query->where('nomabvset', 'MAC');})->whereHas("selections", function($query){$query->where("sitatl","!=","Desligado");})->get() as $schoolclass)
    ...
    {!! $schoolclass->codtur !!} & 
    {!! $schoolclass->nomdis !!} &     
    \makecell[l]{@foreach($schoolclass->requisition->activities as $activity) {!! $activity->description !!}\\ @endforeach}  & 
    {!! $schoolclass->requisition->requested_number !!} & 
    {!! $schoolclass->selections()->where("sitatl","!=","Desligado")->get()->count() !!}
    \\ 
    \midrule
@endforeach
\end{longtable}
