...
@section('title', 'Informar sobre Avaliações')
...
<div id="layout_conteudo">
    <div class="justify-content-center">
        <div class="col-md-12">
            <h1 class='text-center mb-3'>Informar sobre Avaliações</h1>


            <h4 class='text-center mb-5'>{{ $schoolterm->period . ' de ' . $schoolterm->year }}</h4>
            <form method="POST"
                action="{{ route('emails.triggerInstructorEvaluations') }}"
            >
            @csrf

            <p class="alert alert-info rounded-0">
                <b>Atenção:</b>
                Lembre-se que o docente só poderar cadastrar a avaliação de {{ $schoolterm->start_date_evaluations }} à {{ $schoolterm->end_date_evaluations }}.
            </p>
            ...
            @else
                <p class="text-center">Não foram encontradas avaliações por se fazer</p>
            @endif
        </div>
    </div>
</div>
